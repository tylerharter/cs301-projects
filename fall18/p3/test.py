...
TESTS_FUNC = [
    ("testGetNumRecords", [], 529), #1
    ("testGetName", [24], 'ARLENE'), #2
    ("testGetWindSpeed", [38], 25), #3
    ("searchHurricane", ["EMILY"], True), #4
    ("searchHurricane", ["Random1"], False), #5
    ("searchHurricane", ["KENNETH"], True), #6
    ("searchHurricane", ["BORA"], False), #7
    ("searchHurricaneByLocation", ["12.9N","20.5W"], "FAITH"), #8
    ("searchHurricaneByLocation", ["11.2N","95.0W" ], "HENRIETTE"), #9
...
TESTS_PRINT = [
        ["The number of records in the DataSet is ", 529], #1
        ["The name of the hurricane in the 20th row of the dataset is ", "BABE"],
        ["Does the hurricane 'BOB' Exist in the Dataset?", "True"],
        ["Does the hurricane 'BRITNEY' Exist in the Dataset?", "False"],
        ["Find the hurricane that came at 9.0N Latitude and 157.0W Longitude", "JUNE"],
        ["How many hurricanes came from the Pacific?", 241],
        ["How many hurricanes came from the Atlantic?", 288],
        ["What is the difference between the max Windspeed and min Windspeed of the dataSet?", 65]
 ]
"""TESTS_PRINT = [
...
def runFuncTest(functionName, fn, args, expectedResult):
    ...
    if type(expectedResult) != str and abs(actualResult - expectedResult) > EPSILON:
